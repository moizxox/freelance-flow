...
const validateFreelancers = async (users) => {
  ...
  const validFreelancers = await Auth.find({ _id: { $in: ids }, role: "freelancer" });
  if (validFreelancers.length !== users.length) throw new CustomError(400, "One or more freelancer IDs are invalid");
...
const addProject = asyncHandler(async (req, res, next) => {
  ...
  const project = await Project.create({ title, description, totalHours: 0 });
  if (!project) return next(new CustomError(400, "Error while creating project"));

  if (users && users.length > 0) await assignUsersToProject(project, users);
...
const assignFreelancers = asyncHandler(async (req, res, next) => {
  ...
  if (!projectId || !users || !Array.isArray(users)) return next(new CustomError(400, "Project ID and an array of users are required"));
  ...
  return res.status(200).json({ success: true, message: "Freelancers assigned successfully", data: project });
});

// Remove Freelancer
const removeFreelancer = asyncHandler(async (req, res, next) => {
  const { projectId, freelancerId } = req.body;
  if (!projectId || !freelancerId) return next(new CustomError(400, "Project ID and Freelancer ID are required"));
  ...
  if (!userProject) return next(new CustomError(404, "User not assigned to this project"));
  ...
  return res.status(200).json({ success: true, message: "Freelancer removed successfully", data: project });
});

export { addProject, getAllProjects, updateProject, deleteProject, assignFreelancers, removeFreelancer };
